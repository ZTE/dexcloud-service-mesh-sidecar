...
function _M.filter_root_req()
	if ngx_var.uri == "/" then
		if type(route_way) == "string" and not string_find(route_way, "domain", nil, true) then
			return ngx.exec("@defaultpage")
		end
		local m, err = ngx.re.match(ngx_var.host, "^((25[0-5]|2[0-4]\\d|[01]?\\d\\d?)\\.){3}(25[0-5]|2[0-4]\\d|[01]?\\d\\d?)$","o")
		if m then
			return ngx.exec("@defaultpage")
		end
	end
