...
local route_way = msbConf.routerConf.routeway
...
local function _get_key_prefix(server_port)
	...
	if ("microservices" == svc_name or "msdiscover" == svc_name or "route" == svc_name) then
		return defaultprefix
	elseif (server_port == defaultport or server_port == defaulthttpsport) then
		if type(route_way) == "string" and not str_find(route_way, "domain", nil, true) then
			return defaultprefix
		end
...
local function _load_common_svc_info(svc_type,server_port)
	...
	if(svc_is_api_related_types(svc_type)) then
		...
		if svc_name ~= str_low(svc_name) then
			svc_key_bak = tbl_concat({key_prefix,"api",str_low(svc_name),svc_version},":")
		end
		if(version1 and "" ~= version1) then 
			svc_pub_url = "/"..svc_type..version1.."/"..svc_name
		elseif(version2 and "" ~= version2) then 
			svc_pub_url = "/"..svc_type.."/"..svc_name..version2
		else 
		...
		if svc_name ~= str_low(svc_name) then
			svc_key_bak = tbl_concat({key_prefix,"iui",str_low(svc_name)},":")
		end
	...
	if tbl_isempty(svcinfo) then
		if svc_key_bak ~= "" then
			svcinfo = dbclient.load_serviceinfo(svc_key_bak)
		end
		if tbl_isempty(svcinfo) then
...
local function _load_custom_svc_info(svc_type,server_port)
	...
	if not matchedsvcname then 
		local refer =  ngx_var.http_referer
		if(refer and refer~="") then
			for _, svcname in ipairs(svcnames) do
				local urlreg = "^(https://|http://|)([^/]*)"..svcname.."(/(.*))?$";
...
function _M.execute(server_port,system_tag)
	...
	ngx.ctx.origin_uri = ngx_var.uri
