...
local iso_time_str = function(time)
	local d = pl_Date(time)
	d:toUTC ()
	return d:__tostring()
end
...
function _M.collect_req_status()
    local req_status = {}
...
_reset_reqnum_stats = function(premature)
	...
	stats_cache:incr(req_num_stats_key, 0-count)
	local metric = {}
	metric["stats_time"] = iso_time_str(ngx.time())
	metric["responses"] = {total=count} 
	dao.save_reqnum_stats(ngx.today(), cjson_safe.encode(metric))
    --dao.save_reqnum_stats(ngx.today(), iso_time_str(ngx.time()).."|"..count)
