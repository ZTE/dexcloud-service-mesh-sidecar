...
function _M.enable_refer_match(svcinfo)
	...
	--Be compatible with the old service info. If the field is not filled, use the global enablerefercheck value.
	if enable_refer_match == nil or enable_refer_match == "" then
		return enablerefercheck
	else
		return str_low(enable_refer_match) == "true"
	end
end
